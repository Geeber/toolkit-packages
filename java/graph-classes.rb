...
opts = OptionParser.new do |opts|
  ...
  opts.on('-g', "--[no-]group", "Group types together by package") {|v| $group_types = v }
  opts.on('-i', "--[no-]imports", "Show import relationships") {|v| $show_imports = v }
  ...
  def define(declaration)
    ...
    if /^\s*implements\s+(.+)/ === remaining
      ...
      implements.each do |interface|
        itype, remaining = parse_type(interface)
